...
import { Send, Bot, User, Sparkles, ShieldAlert, BookOpen, ExternalLink, Loader2 } from 'lucide-react';
...
export const RagChat: React.FC<RagChatProps> = ({ docId, onCitationClick }) => {
  ...
  return (
    <div className="flex flex-col h-[650px] bg-slate-900/80 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl backdrop-blur-md">
      {/* Header */}
      ...
      <div className="p-3 bg-slate-950/40 border-b border-slate-800/80 flex items-center gap-2 overflow-x-auto scrollbar-none">
        ...
        {SUGGESTED_QUESTIONS.map((q, idx) => (
          <button
            ...
            disabled={loading}
            className="px-2.5 py-1 bg-slate-800/80 hover:bg-slate-800 border border-slate-700/80 text-[11px] text-slate-300 rounded-lg whitespace-nowrap transition-colors"
          >
            {q}
          </button>
        ))}
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {msg.citations.map((cite, cIdx) => (
                      <button
                        ...
                        className="flex items-center gap-1 px-2.5 py-1 bg-slate-900 border border-emerald-500/30 hover:border-emerald-400 text-[10px] text-emerald-300 rounded-lg transition-colors group"
                        title={cite.snippet}
                      >
                        <span>{cite.section_title}</span>
                        <ExternalLink className="w-2.5 h-2.5 text-emerald-400 opacity-60 group-hover:opacity-100" />
                      </button>
                    ))}
                  </div>
                </div>
      ...
      >
        ...
        <button
          ...
          className="p-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl disabled:opacity-50 transition-colors shadow-md shadow-emerald-950/50"
        >
