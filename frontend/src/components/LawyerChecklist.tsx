...
import { HelpCircle, CheckSquare, Square, Printer, Shield, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-slate-900/70 border border-slate-800 rounded-3xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xl backdrop-blur-md">
        ...
        <button
          onClick={handlePrint}
          className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl text-xs font-semibold text-slate-200 transition-colors shadow"
        >
          ...
          questions.map((item, idx) => {
            ...
            return (
              <div
                ...
                className={`cursor-pointer bg-slate-900/70 border rounded-2xl p-5 transition-all duration-200 space-y-3 shadow-md ${
                  isChecked
